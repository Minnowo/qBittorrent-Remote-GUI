...
class ClientController(object):
    def __init__(self):
        ...
        self._caches :dict[str, Cache.Data_Cache | Cache.Expiring_Data_Cache]= {}
    ...
    def call_later(self, initial_delay_seconds: float, func: Callable, *args, **kwargs) -> CoreThreading.Single_Job:
    ...
    def get_torrents(self, skip_cache=False, **kwargs):
        ...
        """
        if not skip_cache and kwargs is None:
    ...
    def update_torrents_file_priority_transactional(self, torret_hash:str, file_id:int, priority:int):
        """
        ...
        if not self.file_priority_transaction_cache.has_data(torret_hash):
            ...
            self.file_priority_transaction_cache.add_data(torret_hash, 
            {
                "file_ids" : [file_id],
                "priority" : priority
            }, True)
            ...
            def callback(torrent_hash):
                ...
                CG.client_instance.torrents_file_priority(torrent_hash,
                                                          data['file_ids'],
                                                          data['priority'])
